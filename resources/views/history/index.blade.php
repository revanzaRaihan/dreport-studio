...
<section class="panel active" id="tab-riwayat">
    <div class="card">
        ...
        <div style="display: flex; align-items: flex-start; justify-content: space-between; flex-wrap: wrap; gap: 10px; margin-bottom: 12px;">
            ...
            @if($student)
                <a href="{{ route('history.index') }}" class="btn secondary" style="padding: 6px 12px; font-size: 12px; text-decoration: none; flex-shrink: 0;">← Semua murid</a>
            @endif
        </div>

        {{-- Search + student filter --}}
        <form method="GET" action="{{ route('history.index') }}" class="search-bar">
            @if($student)
            ...
            @if($students->isNotEmpty())
                <select name="student_id" id="studentFilter" style="margin-bottom:0; flex-shrink:0; width:auto;">
            ...
            @endif
            {{-- Search icon button --}}
            <button type="submit" class="btn-icon" title="Filter">
